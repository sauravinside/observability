...
# Create a systemd service for Flask application
sudo tee /etc/systemd/system/aws-monitoring.service > /dev/null <<EOF
[Unit]
Description=AWS Monitoring Tool
...
ExecStart=sudo /usr/bin/python3 /home/ubuntu/aws-monitoring-tool/backend/app.py
Restart=always
...
sudo systemctl enable aws-monitoring
sudo systemctl start aws-monitoring

echo "Flask application is now running as a systemd service."
